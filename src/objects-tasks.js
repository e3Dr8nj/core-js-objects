...
function mergeObjects(objects) {
  return objects.reduce((merged, obj) => {
    const updates = Object.entries(obj).reduce(
      (acc, [key, value]) => ({
        ...acc,
        [key]: (merged[key] || 0) + value,
      }),
      {}
    );
...
function makeImmutable(obj) {
  const immutableObj = {};
  Object.assign(immutableObj, obj);
...
function makeWord(lettersObject) {
  ...
  const sum = keys.reduce((acc, item) => {
    return acc + lettersObject[item].length;
  }, 0);
  const arr = new Array(sum).fill('');
  keys.forEach((key) => {
...
function Rectangle(width, height) {
  ...
  obj.width = width;
  obj.height = height;
  Object.defineProperty(obj, 'getArea', {
    value() {
